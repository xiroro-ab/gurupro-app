...
export default function App() {
  ...
  // Modal Listener
  useEffect(() => {
    const handleOpenAbout = () => setIsAboutCreatorOpen(true);
    window.addEventListener('open-about-creator', handleOpenAbout);
    return () => window.removeEventListener('open-about-creator', handleOpenAbout);
  }, []);
  ...
  useEffect(() => {
    ...
    const checkConnection = async () => {
      ...
      // Fetch global tahun ajaran
      try {
        const globalYear = await GuruService.getGlobalTahunAjaran();
        if (globalYear) {
          localStorage.setItem('gurupro_tahun_ajaran', globalYear);
          localStorage.removeItem('gurupro_tahun_ajaran_is_custom'); // Clear legacy custom flag
          window.dispatchEvent(new Event('tahun_ajaran_changed'));
        }
      } catch (err) {
        console.warn('Failed to sync global tahun ajaran', err);
      }
    };
    ...
    try {
      ...
      if (cached) {
        ...
        GuruService.getTeacherById(parsed.id).then((updatedProfile) => {
          ...
          console.warn('Error refreshing profile in background:', err);
        });
      }
    } catch (e) {
  ...
  const renderTabContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'pengaturan-sistem':
        ...
        return <PengaturanSistem currentUser={currentUser} />;
  ...
  return (
    <div id="gurupro-workspace" className="min-h-screen bg-slate-50 flex overflow-x-hidden">
      ...
      <div className="flex-1 flex flex-col min-h-screen lg:pl-64 print:pl-0 w-full min-w-0 print-flow-fix">
      ...
      <AboutCreatorModal isOpen={isAboutCreatorOpen} onClose={() => setIsAboutCreatorOpen(false)} />
    </div>
  );
}
