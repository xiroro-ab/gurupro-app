...
export default function AbsensiHarian({ currentUser, mode = 'mapel' }: AbsensiHarianProps) {
  ...
  const [selectedDate, setSelectedDate] = useState('2026-06-23'); // Preset to user context time
  ...
  useEffect(() => {
    ...
    classStudents.forEach(student => {
      ...
      if (match) {
        ...
        if (cleanKet.startsWith('[WK]')) cleanKet = cleanKet.substring(4);
        else if (cleanKet.startsWith('[MP]')) cleanKet = cleanKet.substring(4);
  ...
  }, [classStudentsSerialized, existingAttendancesSerialized, selectedClassId, selectedDate, isWalikelasMode]);
  ...
  const handleSave = () => {
    ...
    const prefix = isWalikelasMode ? '[WK]' : '[MP]';
    const recordsToSave: Omit<DailyAttendance, 'id'>[] = classStudents.map(student => {
      ...
      return {
        ...
        status: state.status,
        keterangan: prefix + (state.keterangan || ''),
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h3 className="text-lg font-bold text-slate-800">
            {isWalikelasMode ? 'Catat Presensi Harian Wali Kelas' : 'Catat Presensi Guru Mapel'}
          </h3>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {isWalikelasMode ? (
            ...
            <select
              ...
              className="px-4 py-2 bg-slate-50 border border-slate-300 rounded-xl focus:outline-hidden focus:ring-2 focus:ring-blue-600 text-sm font-semibold text-slate-700 cursor-pointer"
            >
