...
import { User, Mail, FileText, Award, Calendar, Camera, Save } from 'lucide-react';
...
export default function Profile({ currentUser, onProfileUpdate }: ProfileProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update global Academic Year if admin changed it
      if (currentUser.profile.role === 'admin') {
        localStorage.setItem('gurupro_tahun_ajaran', tahunAjaran);
        // Dispatch custom event so other components know the academic year updated
    ...
    <div id="user-profile-page" className="max-w-4xl mx-auto space-y-8 animate-fadeIn">
      ...
      <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          {/* Admin Academic Year settings card */}
          {currentUser.profile.role === 'admin' && (
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-4">
              <div className="flex items-center gap-2 text-slate-800">
                <Calendar className="h-5 w-5 text-blue-600" />
                <h3 className="text-base font-bold">Konfigurasi Tahun Ajaran</h3>
              </div>
              <p className="text-xs text-slate-500 leading-relaxed">
                Sebagai Administrator, Anda dapat mengatur Tahun Ajaran aktif untuk seluruh tampilan di dasbor utama dan laporan cetak.
              </p>
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1">TAHUN AJARAN SEKARANG</label>
                <input
                  type="text"
                  value={tahunAjaran}
                  onChange={(e) => setTahunAjaran(e.target.value)}
                  placeholder="Contoh: 2025/2026"
                  className="w-full px-3.5 py-2 border border-slate-200 rounded-xl text-sm focus:outline-hidden focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-semibold text-slate-800"
                />
              </div>
            </div>
          )}
