...
export default function ProgresGuru() {
  ...
  const attendanceSessions = React.useMemo(() => {
    ...
    teacherAttendances.forEach(att => {
      ...
      const isKeg = att.keterangan && String(att.keterangan).startsWith('[KEG]');
      const typeLabel = isWK ? 'WK' : isKeg ? 'KEG' : 'MP';
      
      let kegiatanName = '';
      if (isKeg) {
        let rest = String(att.keterangan).substring(5).trim();
        kegiatanName = rest.split(' - ')[0];
      }
      const key = `${att.tanggal}_${classId}_${typeLabel}${isKeg ? `_${kegiatanName}` : ''}`;
  ...
  const handleExportCSV = () => {
    ...
    if (activeSubTab === 'jurnal') {
      ...
      filteredAttendanceSessions.forEach((session, idx) => {
        ...
        const roleLabel = session.type === 'WK' ? 'Wali Kelas' : session.type === 'KEG' ? `Kegiatan: ${session.kegiatanName}` : 'Guru Mapel';
            ...
            <div style={{ display: 'flex', alignItems: 'center', borderBottom: '3px solid black', paddingBottom: '10px', marginBottom: '20px' }}>
              ...
              <img src="https://raw.githubusercontent.com/xiroro-ab/smp58dataguru/refs/heads/main/ico.png" style={{ width: '80px', height: '80px', objectFit: 'contain' }} alt="Logo SMP 58" referrerPolicy="no-referrer" />
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', wordBreak: 'break-word', border: '1px solid black' }}>
                    ...
                    <tbody>
                      {filteredJournals.length === 0 ? (
                        <tr>
                          <td colSpan={filterType !== 'agenda_harian' ? 8 : 7} style={{ border: '1px solid black', padding: '10px', fontSize: '9pt', textAlign: 'center', fontStyle: 'italic' }}>
                        ...
                        filteredJournals.map((j, idx) => {
                          ...
                          return (
                            <tr key={`${keyPrefix}-jurnal-layout-${idx}`}>
                              ...
                              {filterType !== 'agenda_harian' && (<td style={{ border: '1px solid black', padding: '5px', fontSize: '9pt', color: '#9f1239', fontWeight: 'bold' }}>
                                {isMgmp || j.journal_type === 'agenda_harian' ? '-' : (j.siswa_absen || 'Hadir Semua')}
                              </td>)}
                ...
                {activeSubTab === 'absensi' && (
                  <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', wordBreak: 'break-word', border: '1px solid black' }}>
                    <thead>
                      <tr style={{ backgroundColor: '#f2f2f2' }}>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'center', width: '5%' }}>No</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'left', width: '15%' }}>Tanggal Presensi</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'left' }}>Kelas Binaan / Mapel</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'center', width: '12%' }}>Hadir (H)</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'center', width: '12%' }}>Sakit (S)</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'center', width: '12%' }}>Izin (I)</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'center', width: '12%' }}>Alfa (A)</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'center', width: '12%' }}>Rasio</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        filteredAttendanceSessions.map((session, idx) => {
                          ...
                          return (
                            <tr key={`${keyPrefix}-absensi-layout-${idx}`}>
                              ...
                              <td style={{ border: '1px solid black', padding: '5px', fontSize: '9pt' }}>
                                Kelas {className} <span style={{ fontSize: '8pt', color: '#666' }}>({session.type === 'WK' ? 'Wali Kelas' : session.type === 'KEG' ? `Kegiatan: ${session.kegiatanName}` : 'Guru Mapel'})</span>
                              </td>
                ...
                {activeSubTab === 'penilaian' && (
                  <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', wordBreak: 'break-word', marginTop: '10px' }}>
                    <thead style={{ backgroundColor: '#f1f5f9' }}>
                      <tr>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'center', width: '5%' }}>No</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'left', width: '20%' }}>Waktu Terakhir</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'left' }}>Kelas Binaan / Mapel</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'left', width: '15%' }}>Tipe Nilai</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'left', width: '10%' }}>Semester</th>
                        <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'center', width: '15%' }}>Siswa Dinilai</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(() => {
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ border: '1px solid black', padding: '20px' }}>
                          ...
                          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed', wordBreak: 'break-word', fontSize: '10pt' }}>
                            <tbody>
                              <tr>
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="w-40">
                          ...
                          <SearchableSelect
                            ...
                            value={filterClassId}
                            onChange={(val) => setFilterClassId(val)}
                      ...
                      <div className="flex items-center gap-4">
                        <div>
                          ...
                          >
                            ...
                            <option value="KEG">Kegiatan Lain</option>
                          </select>
                        </div>
                                ...
                                <tr key={`view-absensi-${idx}`} className="hover:bg-slate-50/50 transition-colors">
                                  ...
                                  <td className="px-4 py-3">
                                    ...
                                    <span className={`inline-block mt-0.5 text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase ${
                                      session.type === 'WK' ? 'bg-amber-100 text-amber-800 border border-amber-200' : 
                                      session.type === 'KEG' ? 'bg-purple-100 text-purple-800 border border-purple-200' : 
                                      'bg-blue-100 text-blue-800 border border-blue-200'
                                    }`}>
                                      {session.type === 'WK' ? 'Wali Kelas' : session.type === 'KEG' ? `Kegiatan: ${session.kegiatanName}` : 'Guru Mapel'}
                                    </span>
                                  </td>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/75 backdrop-blur-xs flex items-start justify-center p-4 md:p-6 no-print">
          <div className="bg-slate-100 rounded-2xl w-full max-w-[220mm] shadow-2xl overflow-hidden flex flex-col my-8">
            ...
            <div className="overflow-auto p-8 max-h-[70vh] flex justify-center bg-slate-200/50">
              ...
              <div className="bg-white w-[210mm] min-w-[210mm] shrink-0 min-h-[297mm] p-12 shadow-md border border-slate-300 rounded-sm text-black relative flex flex-col items-stretch text-left print:w-full print:min-h-0 print:block print:p-6 print:shadow-none print:border-none print:m-0 print:overflow-visible print:min-w-0 print:w-full">
