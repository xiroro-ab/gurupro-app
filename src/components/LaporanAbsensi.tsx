...
export default function LaporanAbsensi({ currentUser }: LaporanAbsensiProps) {
  ...
  const [reportType, setReportType] = useState<'walikelas' | 'mapel'>(isWalikelas ? 'walikelas' : 'mapel');
  const [selectedClassId, setSelectedClassId] = useState('');
  const [selectedMonth, setSelectedMonth] = useState('2026-06'); // Default to month of context
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    const typeSuffix = reportType === 'walikelas' ? 'Wali_Kelas' : 'Guru_Mapel';
      ...
      <div className="hidden print:block print-area">
        ...
        <div style={{ textAlign: 'center', marginBottom: '20px' }}>
          <h2 style={{ fontSize: '14pt', fontWeight: 'bold', textDecoration: 'underline', margin: 0 }}>
            LAPORAN BULANAN PRESENSI SISWA ({reportType === 'walikelas' ? 'WALI KELAS' : 'GURU MAPEL'})
          </h2>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid black' }}>
          <thead>
            <tr style={{ backgroundColor: '#f2f2f2' }}>
              ...
              <th style={{ border: '1px solid black', padding: '6px', fontSize: '10pt', textAlign: 'left' }}>NISN</th>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select
              ...
              className="px-4 py-2 bg-slate-50 border border-slate-300 rounded-xl text-sm font-semibold text-slate-700 cursor-pointer"
            >
              <option value="">-- Pilih Kelas --</option>
              {filteredClassesList.map(c => (
                <option key={c.id} value={c.id}>
                  Kelas {c.nama_kelas} {isWalikelas && c.walikelas_id === profile.id ? '⭐ (Asuhan Anda)' : ''}
                </option>
              ))}
            </select>

            {/* Teacher filter for Admin in Guru Mapel report mode */}
            {reportType === 'mapel' && profile.role === 'admin' && (
              <select
                value={selectedTeacherId}
                onChange={(e) => setSelectedTeacherId(e.target.value)}
                className="px-4 py-2 bg-slate-50 border border-slate-300 rounded-xl text-sm font-semibold text-slate-700 cursor-pointer max-w-[200px]"
              >
                <option value="">Semua Guru Mapel</option>
                {mapelTeachersList.map(t => (
                  <option key={t.id} value={t.id}>
                    {t.nama_lengkap} ({t.mapel || 'Guru Mapel'})
                  </option>
                ))}
              </select>
            )}
                  ...
                  <table className="w-full border-collapse border border-black text-xs text-black">
                    <thead>
                      <tr className="bg-slate-100">
                        ...
                        <th className="border border-black p-2 text-left font-bold">NISN</th>
