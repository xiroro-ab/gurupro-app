...
export default function KelolaSiswa({ currentUser }: KelolaSiswaProps) {
  ...
  const bulkMutation = useMutation({
    ...
    onError: (err: any) => {
      showNotification(err?.message || 'Gagal melakukan impor massal. Pastikan format CSV valid.', 'error');
    }
  });
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        if (validStudents.length > 0) {
          ...
          // Deduplicate within the CSV itself based on NISN
          const uniqueValidStudents = validStudents.filter((v, i, a) => a.findIndex(t => t.nisn === v.nisn) === i);
          
          const duplicates = uniqueValidStudents.filter(s => currentNisns.includes(s.nisn));

          if (duplicates.length > 0) {
            showNotification(`${duplicates.length} siswa dilewati karena NISN sudah ada di sistem.`, 'error');
          }
          
          const internalDuplicatesCount = validStudents.length - uniqueValidStudents.length;
          if (internalDuplicatesCount > 0) {
            showNotification(`${internalDuplicatesCount} baris duplikat di dalam file CSV dilewati.`, 'error');
          }

          const nonDuplicates = uniqueValidStudents.filter(s => !currentNisns.includes(s.nisn));
          ...
          {isWalikelas ? (
            ...
            <SearchableSelect
              options={[
                ...
                ...classes.map((cls) => ({ value: cls.id, label: `Kelas ${cls.nama_kelas}`, searchStr: cls.nama_kelas }))
              ]}
              value={classFilter}
