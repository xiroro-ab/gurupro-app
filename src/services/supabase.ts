...
class OfflineQueue {
  ...
  static add(type: OfflineQueueItem['type'], action: 'create' | 'save', data: any, description: string): void {
...
export const GuruService = {
  ...
  getStudentPhones(): Record<string, string> {
    try {
      ...
      console.error('Error reading student phones:', e);
      return {};
    }
  },
    ...
    for (const item of queue) {
      try {
        if (item.type === 'journal') {
          ...
          const isWK = attendances[0]?.keterangan?.startsWith('[WK]');
        ...
        console.error('Failed to sync offline queue item:', item, err);
  ...
  async getTeachers(): Promise<TeacherProfile[]> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching teachers, falling back to Local Storage', err);
  ...
  async getTeacherById(id: string): Promise<TeacherProfile | null> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching teacher by id:', err);
      }
    }
  ...
  async createTeacher(
    ...
    if (isSupabase && email && password) {
      try {
        ...
        console.error('Supabase auth error creating user for teacher:', authErr);
    ...
    if (isSupabase) {
      try {
        ...
        console.error('Supabase error creating teacher, falling back to Local Storage', err);
  ...
  async updateTeacher(id: string, updates: Partial<TeacherProfile>): Promise<TeacherProfile> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error updating teacher, falling back to Local Storage', err);
      }
    }
    const current = LocalDB.getTeachers();
  ...
  async deleteTeacher(id: string): Promise<void> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error deleting teacher', err);
        throw err;
      }
    }
    const current = LocalDB.getTeachers();
  ...
  async getClasses(): Promise<ClassRoom[]> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching classes, falling back to Local Storage', err);
  ...
  async createClass(classroom: Omit<ClassRoom, 'id'>): Promise<ClassRoom> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error creating class, falling back to Local Storage', err);
      }
    }
  ...
  async updateClass(id: string, updates: Partial<ClassRoom>): Promise<ClassRoom> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error updating class, falling back to Local Storage', err);
      }
    }
    const current = LocalDB.getClasses();
  ...
  async deleteClass(id: string): Promise<void> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error deleting class', err);
        throw err;
      }
    }
    const current = LocalDB.getClasses();
  ...
  async getStudents(): Promise<Student[]> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching students, falling back to Local Storage', err);
  ...
  async createStudent(student: Omit<Student, 'id'>): Promise<Student> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error creating student, falling back to Local Storage', err);
      }
    }
  ...
  async bulkInsertStudents(students: Omit<Student, 'id'>[]): Promise<Student[]> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error bulk inserting students, falling back to Local Storage', err);
      }
    }
    const current = LocalDB.getStudents();
  ...
  async updateStudent(id: string, updates: Partial<Student>): Promise<Student> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error updating student, falling back to Local Storage', err);
      }
    }
    const current = LocalDB.getStudents();
  ...
  async getAttendances(date?: string): Promise<DailyAttendance[]> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching attendances, falling back to Local Storage', err);
  ...
  async saveAttendances(attendances: Omit<DailyAttendance, 'id'>[]): Promise<DailyAttendance[]> {
    ...
    if (activeStorageMode === 'supabase' && date) {
      try {
        ...
        console.error('Supabase error saving attendances, falling back to Local Storage', err);
        const typeStr = isWK ? 'Wali Kelas' : 'Guru Mapel';
    ...
    if (date) {
      ...
      const filtered = current.filter(a => {
        ...
        return isWK ? !itemIsWK : itemIsWK; // if saving WK, keep mapel. If saving mapel, keep WK.
      });
  ...
  async getJournals(): Promise<TeachingJournal[]> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching teaching journals, falling back to Local Storage', err);
  ...
  async createJournal(journal: Omit<TeachingJournal, 'id'>): Promise<TeachingJournal> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error creating journal, falling back to Local Storage', err);
  ...
  async deleteJournal(id: string): Promise<void> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error deleting journal', err);
        throw err;
      }
    }
    const current = LocalDB.getJournals();
  ...
  async getGrades(classId?: string, mapel?: string): Promise<StudentGrade[]> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching grades, falling back to Local Storage', err);
  ...
  async saveGrades(grades: Omit<StudentGrade, 'id' | 'created_at'>[]): Promise<StudentGrade[]> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error saving grades, falling back to Local Storage', err);
  ...
  async getHolidays(): Promise<any[]> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching holidays:', err);
  ...
  async saveHoliday(holiday: any): Promise<void> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error saving holiday:', err);
  ...
  async deleteHoliday(id: string): Promise<void> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error deleting holiday:', err);
      }
    }
    const current = LocalDB.getHolidays();
  ...
  async getSchedules(): Promise<any[]> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching schedules:', err);
  ...
  async saveSchedule(schedule: any): Promise<void> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error saving schedule:', err);
  ...
  async deleteSchedule(id: string): Promise<void> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error deleting schedule:', err);
      }
    }
    const current = LocalDB.getSchedules();
    const filtered = current.filter(s => s.id !== id);
  ...
  async getAnnouncements(): Promise<any[]> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching announcements:', err);
  ...
  async saveAnnouncement(announcement: any): Promise<void> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error saving announcement:', err);
  ...
  async deleteAnnouncement(id: string): Promise<void> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error deleting announcement:', err);
      }
    }
    const current = LocalDB.getAnnouncements();
  ...
  async getCounselingRecords(): Promise<any[]> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error fetching counseling records:', err);
  ...
  async saveCounselingRecord(record: any): Promise<void> {
    ...
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error saving counseling record:', err);
  ...
  async deleteCounselingRecord(id: string): Promise<void> {
    if (activeStorageMode === 'supabase') {
      try {
        ...
        console.error('Supabase error deleting counseling record:', err);
      }
    }
    const current = LocalDB.getCounselingRecords();
  ...
  async getGlobalSchedule(): Promise<any> {
    if (activeStorageMode === 'supabase') {
      try {
        const { data, error } = await supabase.from('global_schedules').select('*').limit(1).single();
    ...
    if (activeStorageMode === 'supabase') {
      try {
        if (!schedule) {
          ...
          const { error } = await supabase.from('global_schedules').delete().neq('id', '');
          if (error) throw error;
        } else {
          const processed = {
            ...schedule,
            id: 'global_sch_1' // Always use a single ID for the global schedule to overwrite
          };
          const { error } = await supabase.from('global_schedules').upsert([processed]);
          if (error) throw error;
        }
      } catch (err) {
        console.error('Supabase error setting global schedule:', err);
      }
    }
